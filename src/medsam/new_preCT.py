...
parser.add_argument('-i', '--png_path', type=str, default='data/train/png', help='path to the png images')
parser.add_argument('-gt', '--gt_path', type=str, default='data/train/gt', help='path to the ground truth',)
...
parser.add_argument('--img_name_suffix', type=str, default='_0000.png', help='image name suffix')
...
names = [name for name in names if os.path.exists(join(args.png_path, name.split('.nii.gz')[0] + args.img_name_suffix))]
...
def preprocess_ct(gt_path, png_path, gt_name, image_name, label_id, image_size, sam_model):
    ...
    if np.sum(gt_data)>1000:
        ...
        # Read PNG image
        image_data = io.imread(os.path.join(args.png_path, image_name))

        # Preprocess image_data as in the original script
        lower_bound = -500
        upper_bound = 1000
        ...
        for i in range(gt_data.shape[0]):
            ...
            if np.sum(gt_slice_i)>100:
                ...
                if sam_model is not None:
                    ...
                    assert input_image.shape == (1, 3, sam_model.image_encoder.img_size, sam_model.image_encoder.img_size), 'input image should be resized to the correct size'
                    with torch.no_grad():
